...
export class CandyStore {
  ...
  @action
  fetch() {
    this.candyApi.list().subscribe(candies => (this.candies = candies));
  }
  ...
  add(candy: Candy) {
    this.candyApi
      .add(candy)
      .subscribe(id =>
        this.candyApi.get(id).subscribe(added => this.candies.push(added))
      );
  }
  ...
  update(candy: Candy) {
    this.candyApi
      ...
      .subscribe(
        () =>
          (this.candies = this.candies.map(
            c => (c.id === candy.id ? candy : c)
          ))
      );
  }
