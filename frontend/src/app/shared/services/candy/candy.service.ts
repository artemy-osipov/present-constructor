...
export class CandyService {
  ...
  constructor(
    private store: CandyStore,
    private gateway: CandyGateway
  ) { }

  list(): Observable<Candy[]> {
    return this.gateway.list().pipe(
      tap(candies => this.store.set(candies))
    );
  }

  getCandy(id: ID): Observable<Candy> {
    return this.gateway.get(id).pipe(
      tap(candy => this.store.upsert(id, candy))
    );
  }
  ...
  update(candy: Candy): Observable<Object> {
    return this.gateway.update(candy).pipe(
      tap(_ => this.store.update(candy.id, candy))
    );
  }
