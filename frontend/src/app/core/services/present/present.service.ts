...
export class PresentService {
  constructor(private store: PresentStore, private gateway: PresentGateway) {}

  list(): Observable<Present[]> {
    return this.gateway.list().pipe(tap((presents) => this.store.set(presents)))
  }

  getPresent(id: ID): Observable<Present> {
    return this.gateway
      .get(id)
      .pipe(tap((present) => this.store.upsert(id, present)))
  }
